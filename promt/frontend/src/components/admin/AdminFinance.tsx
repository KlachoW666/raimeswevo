...
import { ArrowDownLeft, ArrowUpRight, Check, X, Sliders } from 'lucide-react';
import { useState } from 'react';

type TabType = 'transactions' | 'limits';

export default function AdminFinance() {
    const { transactions, updateTransactionStatus, settings, updateSettings, addAuditEntry } = useAdminStore();
    const { hapticFeedback, showAlert } = useTelegram();
    const [tab, setTab] = useState<TabType>('transactions');
    const [filter, setFilter] = useState<'all' | 'pending' | 'deposit' | 'withdraw'>('all');
    ...
    const handleApprove = (id: string) => {
        ...
        addAuditEntry({ adminId: 'admin', action: 'Одобрение вывода', details: `TX: ${id}` });
        if (showAlert) showAlert('Транзакция одобрена');
    };

    const handleReject = (id: string) => {
        updateTransactionStatus(id, 'rejected');
        hapticFeedback?.notificationOccurred('warning');
        addAuditEntry({ adminId: 'admin', action: 'Отклонение вывода', details: `TX: ${id}` });
        if (showAlert) showAlert('Транзакция отклонена');
    };
            ...
            {tab === 'transactions' ? (
                <>
                    ...
                    <div className="flex gap-2 overflow-x-auto no-scrollbar">
                        {(['all', 'pending', 'deposit', 'withdraw'] as const).map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                            ...
                            >
                                {f === 'all' ? 'Все' : f === 'pending' ? '⏳ Ожидание' : f === 'deposit' ? '↓ Депозиты' : '↑ Выводы'}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="space-y-2">
                        {filteredTx.map(tx => (
                            <div key={tx.id} className="bg-[#161B22] border border-[#30363D] rounded-xl p-4">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${tx.type === 'deposit' ? 'bg-[#00D26A]/10 text-[#00D26A]' : 'bg-[#FF6B6B]/10 text-[#FF6B6B]'}`}>
                                            {tx.type === 'deposit' ? <ArrowDownLeft size={16} /> : <ArrowUpRight size={16} />}
                                        </div>
                                        <div>
                                            <div className="text-sm font-bold text-white">{tx.userName}</div>
                                            <div className="text-[10px] text-[#8B949E] font-mono">{tx.userId}</div>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <div className={`text-base font-bold font-mono ${tx.type === 'deposit' ? 'text-[#00D26A]' : 'text-[#FF6B6B]'}`}>
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {tx.status === 'pending' && (
                                        <div className="flex gap-2">
                                            <button onClick={() => handleApprove(tx.id)} className="w-8 h-8 rounded-lg bg-[#00D26A]/10 flex items-center justify-center text-[#00D26A] active:scale-90 transition-transform">
                                                <Check size={16} />
                                            </button>
                                            <button onClick={() => handleReject(tx.id)} className="w-8 h-8 rounded-lg bg-[#FF4444]/10 flex items-center justify-center text-[#FF4444] active:scale-90 transition-transform">
                                                <X size={16} />
                                            </button>
                                        </div>
                                    )}
                        ...
                        {filteredTx.length === 0 && (
                            <div className="text-center text-[#8B949E] py-10 text-sm">Нет транзакций</div>
                        )}
                    </div>
                </>
            ) : (
